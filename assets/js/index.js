...
function Register() {
    try {
        ...
        $.ajax({
            url: localStorage.getItem("APIURL") + "/Values/RegisterCustmoer?name=" + Name + "&Add=" + Address + "&email=" + Email + "&phone=" + MoNo + "&Pass= &AppType=" + APPType + "&PCODE=" + PCODE + "&Stcode=" + state + "&CtCode=" + city + "&area=" + area + "&deviceId =" + localStorage.getItem("DeviceId") + "&DrName=" + DrName + "&DrCode=" + DrCode,
            type: "GET",
            dataType: "json",
            cache: false,
            success: function (data) {
        ...
        $('#itm-srch').autocomplete({
            ...
            select: function (event, ui) {
              ...
              // 
                ...
                $("#mfd_by").text(ui.item.mfd_by);
                $("#ptcode").text(ui.item.pcode);

                $("#ptname").text(ui.item.party);
                $("#company_name").text(ui.item.company_name);
                $("#lblItmMRP").text(ui.item.Icode);
                $("#lblItmName").text(ui.item.label);
                $("#PackExp").text(ui.item.packing);
                $("#lblContent").text(ui.item.GNAME);
                
                item_pt = ui.item.Icode;
                GET_PARTYCODE(item_pt);
                fun_showItmInfo("#");
            },
            close: function () {
            },
       ...
       // url: WebSerUrl,
        ...
        success: function (data) {
            ...
            for (var i = 0; i < itm.length; i++) {
                ...
                if (i == itm.length - 1) {
                    $("#PT_BODY tr:last").after("<tr><td colspan='' style='font-family: Verdana;color:black;font-weight:bold;width:70px;'>" + itm[i].PCODE + "</td><td align='center' style='color:black;font-weight:bold; text-align:center;'>" + itm[i].PNAME + "</td></tr></tbody>");

                } else {
                  
                    $("#PT_BODY tr:last").after("<tr><td colspan='' style='font-family: Verdana;color:black;font-weight:bold;width:70px;'>" + itm[i].PCODE + "</td><td style='color:black;font-weight:bold; text-align:center;'>" + itm[i].PNAME + "</td></tr></tbody>");
                }
            }
            getparty()
        },
...
function getparty(party_all)
{
    
    var WebSerUrl = localStorage.getItem("APIURL");
    url = GBCServicePath + "/Values/PARTY_all?PCODE1=" + party_all;
    $.ajax({
        url: url,
        // url: WebSerUrl,
        type: "GET",
...
function GetDataFromMobNo() {
    WebSerUrl = localStorage.getItem("APIURL");
    $.ajax({
        url: WebSerUrl + "/Values/getDataFromMobno?MobNo=" + $("#txt-clientMobno").val(),
        type: "GET",
        dataType: "json",
        cache: false,
        success: function (data) {
